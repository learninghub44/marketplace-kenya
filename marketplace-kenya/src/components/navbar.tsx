...
import { ShoppingBag, Search, Moon, Sun, Menu, X, LogOut, User } from 'lucide-react'
...
const CATS = ['📱 Electronics','👗 Fashion','🏡 Home & Garden','🚗 Vehicles','🌾 Agriculture','⚽ Sports','🧸 Baby & Kids','🏢 Property','💼 Services']
...
export default function Navbar({ onSearch, showSearch = false }: NavbarProps) {
  ...
  return (
    <nav className="bg-gray-900 dark:bg-gray-950 sticky top-0 z-50 shadow-lg w-full">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {user ? (
                <>
                  ...
                  <button onClick={logout} className="p-2 text-gray-400 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors">
                    <LogOut className="h-4 w-4" />
                  </button>
                </>
              ) : (
                <>
                  <Link href="/login" className="text-xs text-gray-300 hover:text-white px-2 py-1.5 hover:bg-gray-800 rounded-lg transition-colors">Login</Link>
                  <Link href="/register" className="bg-orange-400 hover:bg-orange-500 text-gray-900 font-bold px-3 py-1.5 rounded-lg text-xs transition-colors whitespace-nowrap">Sign Up</Link>
                </>
              )}
            </div>
      ...
      {/* Category strip - scrollable, no wrapping */}
        ...
        <div className="sm:hidden bg-gray-900 dark:bg-gray-950 border-t border-gray-800 px-3 py-3 space-y-1">
          ...
          {user ? (
            ...
            <>
              ...
              <Link href="/register" onClick={() => setMenuOpen(false)} className="block bg-orange-400 text-gray-900 font-bold px-3 py-2 rounded-lg text-center text-sm">Create Free Account</Link>
            </>
          )}
        </div>
      )}
